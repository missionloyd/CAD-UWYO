...
def generate_buildings(zone_all, API_url, altitude_default=0,
    ...
    for MO_index in zone_all.index:
        ...
        # Status flags for simulation defaults
        Simulate_status = True

        # Determine the construction year and threshold values
        construction_year = row.get('construction_year')
        if not construction_year:  # Set to most recent year if unspecified
            construction_year = max(dicts.THRESHOLDS.keys())
        else:
            # Find the first year that matches or exceeds the specified construction year
            for year in sorted(dicts.THRESHOLDS.keys()):
                if construction_year <= year:
                    construction_year = year
                    break

        # Get the parameters for the identified construction year
        ...
        buildings.append( 
            { 
            'bid':bid, 'geometry':footprint, 'footprint':footprint, 'altitude':altitude, 'height':height,
